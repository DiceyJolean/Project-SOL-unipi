...
//bug: mancavano le funzioni di free, nonostante cancelli la chiave e la restituisca
// in caso di olddata==NULL, faccio cancellare il vecchio dato invece di restituirlo
/**
 ...
 * @returns pointer to the new item.  Returns NULL on error.
 */
icl_entry_t* icl_hash_update_insert(icl_hash_t *ht, void* key, void *data, void **olddata,void (*free_key)(void*), void (*free_data)(void*)){
    ...
    for ( prev=NULL, curr=ht->buckets[hash_val]; curr != NULL; prev=curr, curr=curr->next)
        ...
        if ( ht->hash_key_compare(curr->key, key)) {
            ...
            if (*free_key && curr->key)
                (*free_key)(curr->key);//bug: free(curr->key);
            curr->key=key;
            curr->data=data;
            return curr;
            
            /* il rimuovere l'oggetto e poi reinserirlo può causare inconsistenza nella hash
            nel caso si presenti un errore nella malloc prima dell'inserimento
            if (prev == NULL)
...
/*  non so perchè lo faccia, ma crea sicuramente memory leaks
